...
export function globToRegex(absForwardPattern: string): RegExp {
...
async function globPattern(pattern: string): Promise<string[]> {
 ...
 */
export async function resolveTrxFiles(pattern: string): Promise<string[]> {
...
export function toActionOutputPath(
  filePath: string,
  { cwd = process.cwd(), pathModule = path }: { cwd?: string; pathModule?: typeof path } = {}
): string {
...
export async function convert(): Promise<{ artifactDir: string; freshPlaylists: string[] }> {
